...
class ProductRepository(
    ...
    suspend fun addProduct(product: Product) {
        ...
        val products = snapshot.children.mapNotNull { it.getValue(Product::class.java) }.toMutableList()
        products.add(newProduct)
        productRef.setValue(products).await()
    }
    ...
    suspend fun deleteProduct(id: Int) {
        val snapshot = productRef.get().await()
        val products = snapshot.children.mapNotNull { it.getValue(Product::class.java) }.toMutableList()
        val updatedProducts = products.filter { it.id != id }
        productRef.setValue(updatedProducts).await()
    }
    ...
    private fun initializeDefaultProducts() {
        productRef.get().addOnSuccessListener { snapshot ->
            if (!snapshot.exists()) {
                ...
                productRef.setValue(defaultProducts)
